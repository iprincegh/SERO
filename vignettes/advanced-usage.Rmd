...
for (i in seq_along(buffer_values)) {
  result <- sero_hotspots(data$accident, buffer = buffer_values[i])
...
weight_scenarios <- list(
  "Accident-focused" = list(
    weights = list(accident_weight = 0.6, population_weight = 0.15, road_weight = 0.1),
  ),
  "Population-focused" = list(
    weights = list(accident_weight = 0.2, population_weight = 0.5, road_weight = 0.15),
  ),
  "Balanced" = list(
    weights = list(accident_weight = 0.3, population_weight = 0.25, road_weight = 0.2),
  )
)
...
for (i in seq_along(weight_scenarios)) {
  ...
  result <- do.call(sero_optimal, c(list(data = data, max_locations = 5), params))
...
suburban_locations <- sero_optimal(
  ...
  # Legacy parameter: weight_distance = 0.05,
  # Legacy parameter: weight_distribution = 0.05
)
...
criteria_tests <- list(
  ...
  "Safety-focused" = list(
    weights = list(accident_weight = 0.5, population_weight = 0.1, road_weight = 0.15),
  ),
  "Coverage-focused" = list(
    weights = list(accident_weight = 0.2, population_weight = 0.4, road_weight = 0.1),
  )
)
...
for (scenario_name in names(criteria_tests)) {
  ...
  result <- do.call(sero_optimal, c(list(data = data, max_locations = 3), params))
...
optimal_locs <- sero_optimal(data, max_locations = 5)
all_routes <- sero_routes(optimal_locs, data$accident, max_routes = 50)
